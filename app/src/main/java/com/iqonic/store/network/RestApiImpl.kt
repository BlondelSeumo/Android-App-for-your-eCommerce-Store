...
class RestApiImpl(s: String) {
    ...
    fun listAllProductAttribute(
        onApiSuccess: (ArrayList<StoreProductAttribute>) -> Unit,
        onApiError: (aError: String) -> Unit
    ) {
        getRestApis.getProductAttribute()
            .enqueue(object : Callback<ArrayList<StoreProductAttribute>> {
                override fun onResponse(
                    call: Call<ArrayList<StoreProductAttribute>>,
                    response: Response<ArrayList<StoreProductAttribute>>
                ) {
                    successCallback(onApiSuccess, onApiError, response, call.request())
                }

                override fun onFailure(call: Call<ArrayList<StoreProductAttribute>>, t: Throwable) {
                    failureCallback(onApiError, call.request(), t.toString())
                }
            })
    }
    ...
    fun listAllCategory(
        ...
        getRestApis.listAllCategory(parentId).enqueue(object : Callback<ArrayList<Category>> {
            override fun onResponse(

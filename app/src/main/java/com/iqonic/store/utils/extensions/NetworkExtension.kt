...
import com.iqonic.store.ShopHopApp.Companion.restApiImpl
...
fun AppBaseActivity.listAllProducts(onApiSuccess: (ArrayList<StoreProductModel>) -> Unit) {
    if (isNetworkAvailable()) {
        showProgress(true)
        getRestApiImpl().listAllProducts(onApiSuccess = {
            showProgress(false)
            onApiSuccess(it)
        }, onApiError = {
            showProgress(false)
...
fun getRestApiImpl(url: String = ShopHopApp.getServerUrl()): RestApiImpl {
...
fun AppBaseActivity.listAllCategory(parentId: Int, onApiSuccess: (ArrayList<Category>) -> Unit) {
    if (isNetworkAvailable()) {
        showProgress(true)
        getRestApiImpl().listAllCategory(parentId, onApiSuccess = {
            showProgress(false)
            onApiSuccess(it)
        }, onApiError = {
            showProgress(false)
...
fun AppBaseActivity.fetchAndStoreCartData(
